        ...
        for epoch_counter in range(self.args.epochs):#2
            ...
            for images, _ in tqdm(train_loader):#batch=256,512
                ...
                if epoch_counter  ==0:
                    ...
                    #top_list=list(map(score_list.index, heapq.nlargest(160, score_list)))
                    bottom_list=list(map(score_list.index, heapq.nsmallest(40, score_list)))
                    #random_list=list(np.random.randint(1,195,size=160))
                    print('this is score for {}'.format(len(bottom_list)))
                    batch_list=bottom_list
            ...
            print('Epoch {} takes {} sec to train'.format(epoch_counter,cost_time))
            ...
            print('acc/top1', float(top1[0]))
            print('acc/top5',float(top5[0]))
            print('learning_rate', self.scheduler.get_lr()[0])
            print('--------------------------------------------------------------')
            top1_list.append(float(top1[0]))
            top5_list.append(float(top5[0]))
            ...
            save_checkpoint({
                ...
                'acc/top1':top1[0],
                'acc/top5':top5[0]
            }, is_best=False, filename='D:/tracin-main/S_model/Bottom/20%/'+checkpoint_name)
            print('this checkpoint is about {}epoch and {}step'.format(epoch_counter,n_iter))
            # warmup for the first 10 epochs
            if epoch_counter >= 10:
                self.scheduler.step()
        df.to_csv('D:/tracin-main/loss_in_tracin_bottom_20%.csv')
        df_epoch=pd.DataFrame()
        df_epoch['epoch_loss']=epoch_loss
        df_epoch['top1_acc']=top1_list
        df_epoch['top5_acc']=top5_list
        df_epoch['cost_time']=time_list
        df_epoch.to_csv('D:/tracin-main/pretrain_loss_acc_bottom20%.csv')
